...
void simple_plots(){
  ...
  TFile *inFile = new TFile("/Users/anavrera/MicroBooNE/data/prodgenie_bnb_nu_uboone_overlay_mcc9.1_v08_00_00_26_filter_run3_reco2_G_reco2.root");
  TTree *events = (TTree*) inFile->Get("nuselection/NeutrinoSelectionFilter");
  

  //Create the histograms that will be filled later
  TH1 *h_nu_ene = new TH1F("h_nu_ene","Neutrino energy", 100,0,10);
  TH1 *h_nu_ene_nue = new TH1F("h_nu_ene_nue","Neutrino energy (nue only)", 100,0,10);
  ...
  TCanvas *c_ene = new TCanvas("c_ene","c_ene",1);
  ...
  TCanvas *c_ene_nue = new TCanvas("c_ene_nue","c_ene_nue",1);
